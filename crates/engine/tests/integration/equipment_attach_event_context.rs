...
fn grip_of_phyresis_steals_selected_equipment_and_keeps_its_boosted_germ_alive() {
    ...
    let selected = scenario
        .add_artifact_from_oracle(
            P1,
            "Selected Grip Equipment",
            "Equipped creature gets +0/+1.",
        )
        .with_subtypes(vec!["Equipment"])
        .id();
    let decoy = scenario
        .add_artifact_from_oracle(P0, "Decoy Grip Equipment", "Equipped creature gets +0/+2.")
        .with_subtypes(vec!["Equipment"])
        .id();
    let grip = scenario
    ...
    assert!(outcome.state().objects[&germ]
        .attachments
        .contains(&selected));
...
fn grip_of_phyresis_attaches_before_zero_toughness_germ_dies() {
    ...
    let selected = scenario
        ...
        .id();
    let decoy = scenario
        .add_artifact_from_oracle(P0, "Unchanged Grip Decoy", "Equipped creature gets +0/+1.")
        .with_subtypes(vec!["Equipment"])
        .id();
    let grip = scenario
        .add_spell_to_hand_from_oracle(P0, "Grip of Phyresis", true, GRIP_OF_PHYRESIS_ORACLE)
    ...
    let germ = outcome
        .find_object(|object| {
            object.is_token && object.name == "Phyrexian Germ" && object.zone == Zone::Graveyard
        })
        .expect("unboosted 0/0 Germ must be in its owner's graveyard");
    let attach_index = outcome.events().iter().position(|event| matches!(
        ...
        GameEvent::EffectResolved { kind: engine::types::ability::EffectKind::Attach, source_id, .. }
            if *source_id == grip
    )).expect("Grip's Attach effect must resolve: {:?}", outcome.events());
    let germ_dies_index = outcome.events().iter().position(|event| matches!(
        ...
        GameEvent::ZoneChanged { object_id, from: Some(Zone::Battlefield), to: Zone::Graveyard, .. }
            if *object_id == germ
    )).expect("created Germ must move battlefield to graveyard: {:?}", outcome.events());
